		...
		<div>
			...
			>
				<Tab eventKey="cart" title="Cart">
					<Button variant="danger offset-10" onClick={() => clearCart()}>
						Clear Cart
					</Button>
					{cart.length > 0 ? (
						...
						<>
							<div className="d-flex m-auto justify-content-center height-4">
								<svg
							...
							<div className="d-flex mt-5 m-auto justify-content-center">
				...
				<Tab eventKey="checkout" title="Checkout">
					<div className="m-auto m-5 p-3 w-50">
						<h4>
							You have <strong>{totalQty}</strong> items in your cart !
						</h4>
						{cart.map((item) => (
							<Card className="mx-2 my-1 d-flex flex-row">
								<strong style={{ width: "300px", marginLeft: "5px" }}>
									{item.name}
								</strong>

								<span className="offset-5">
									Rs. {item.price} x <strong>{item.qty}</strong>
								</span>
							</Card>
						))}
						<hr />
